 ...
 */
@Service
public class ElinpcwyRechargePrcodess extends AbstractRechargeProcess {
	...
	public String notify(String notifyData) {
		Map<String,String> map=getMap(notifyData);
          if(!this.verifySign(notifyData)) {
        	  logger.error("银联网银验证签名结果[失败],通知的参数为{}",notifyData);
        	  return "200";
          }
	...
	public boolean syncResponse(String requestData){
		try{
			...
			if(!SDKUtil.validate(map,validateCertDir)) {
				logger.error("网银验证签名结果[失败],requestdate={},signature={}",requestData,stringSign);
				return false;
			}
			...
			if("00".equals(trade_status)){
				this.rechargeResult(out_trade_no, trade_no, new BigDecimal(total_fee).divide(new BigDecimal(100)), true, respMsg);
				return true;
			}else{
