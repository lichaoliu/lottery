...
public class SecureUtil {
	...
	protected static Logger logger = LoggerFactory.getLogger(SecureUtil.class);
	...
	public static String trace(byte[] inBytes) {
		...
		for (int i = 0; i < inBytes.length; i++) {
			if (j == 0) {
				System.arraycopy(String.format("%03d: ",
						new Object[] { Integer.valueOf(i) }).getBytes(), 0,
						temp, 0, 5);
				System.arraycopy(String.format(":%03d",
						new Object[] { Integer.valueOf(i + 15) }).getBytes(),
						0, temp, 72, 4);
			}
			System.arraycopy(String.format("%02X ",
					new Object[] { Byte.valueOf(inBytes[i]) }).getBytes(), 0,
