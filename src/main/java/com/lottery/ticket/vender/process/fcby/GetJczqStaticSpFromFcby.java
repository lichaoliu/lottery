...
@Service
public class GetJczqStaticSpFromFcby extends GetJczqDataFromFcby {

	final static public String QUERY_STATIC_SP_LIST_CODE = "121";
	@Autowired
	...
	protected void generateRootElement(Map<String, String> reqMap,Document returnDoc, IVenderConfig config) throws Exception {
		...
		queryElement.addAttribute("valueType", "1"); //动态sp值查询为0，静态sp值查询为1
		
		xo.write(bodyContentElement);
		xo.close();
		String queryStr = sw.toString();
		agentIdElment.setText(config.getAgentCode());
		cmdElement.setText(QUERY_STATIC_SP_LIST_CODE);
