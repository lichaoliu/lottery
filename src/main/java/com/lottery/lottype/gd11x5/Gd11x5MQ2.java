...
public class Gd11x5MQ2 extends Gd11x5X{
	
	private Gd11x5SQ2 sq2 = new Gd11x5SQ2();
	...
	public String caculatePrizeLevel(String betcode, String wincode,
		...
		if(betcodes[0].contains(wincode.substring(0, 2))&&betcodes[1].contains(wincode.substring(3, 5))) {
			return LotteryDrawPrizeAwarder.GD11X5_Q2.value;
		}
	...
	public List<SplitedLot> splitByType(String betcode, int lotmulti,
		...
		for(SplitedLot s:list) {
			if(s.getBetcode().startsWith("200542")) {
		...
		if(bets.size()==total) {
			...
			for(String bet1:bet1s) {
				for(String bet2:bet2s) {
					if(!bet1.equals(bet2)) {
						SplitedLot slot = new SplitedLot(lotterytype);
						slot.setBetcode("200532-"+bet1+","+bet2+"^");
