...
public class Xjssc04 extends XjsscX{
	
	//选4中4 中一个一等两个二等 加起来一起扣税

	@Override
	public String caculatePrizeLevel(String betcode, String wincode,
			int oneAmount) {
		String win = wincode.substring(2);
		StringBuilder prize = new StringBuilder();
		
		for(String code:betcode.split("-")[1].split("\\^")) {
			if(code.equals(win)) {
				prize.append(LotteryDrawPrizeAwarder.XJSSC_4DALL.value).append(",");
			}else if(code.substring(2).equals(win.substring(2))) {
				prize.append(LotteryDrawPrizeAwarder.XJSSC_4D2.value).append(",");
			}else if(code.subSequence(0, 5).equals(win.subSequence(0, 5))) {
				prize.append(LotteryDrawPrizeAwarder.XJSSC_4D2.value).append(",");
			}
		}
	...
	private List<SplitedLot> transform(String betcode,int lotmulti) {
		...
		for(int i=0;i<betcodes.length;i++) {
			...
			if(sb.toString().split("\\^").length==5) {
				...
				slot.setBetcode("101404-"+sb.toString());
