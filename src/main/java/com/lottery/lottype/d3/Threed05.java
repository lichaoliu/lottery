...
public class Threed05 extends ThreedX{
	...
	public String caculatePrizeLevel(String betcode, String wincode,int oneAmount) {
		...
		for(String code:betcode.split("-")[1].split("\\^")) {
			int match = 0;
			String[] bets = code.split(",");
			for(int i=0;i<bets.length;i++) {
				if(wincodes[i].equals(bets[i])) {
					match = match + 1;
				}
			}
			if(match==2) {
				sb.append(LotteryDrawPrizeAwarder.D3_2D.value).append(",");
			}
		}
		check2delete(sb);
		return sb.toString();
	...
	public long getSingleBetAmount(String betcode, BigDecimal beishu,
		...
		for(String code:betcode.split("\\-")[1].split("\\^")) {
			int select = 0;
			for(String bet:code.split(",")) {
				if(bet.matches("0[0-9]")) {
					select = select + 1;
				}
			}
			if(select!=2) {
