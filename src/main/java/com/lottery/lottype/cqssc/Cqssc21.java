...
public class Cqssc21 extends CqsscX{

	@Override
	public String caculatePrizeLevel(String betcode, String wincode,
			int oneAmount) {
		String[] wins = wincode.split(",");
		int sum = Integer.parseInt(wins[3])+Integer.parseInt(wins[4]);
		
		for(String code:betcode.replace("^", "").split("-")[1].split(",")) {
			if(code.equals(String.valueOf(sum))) {
				return LotteryDrawPrizeAwarder.CQSSC_H2.value;
			}
		}
	...
	public long getSingleBetAmount(String betcode, BigDecimal beishu,
		...
		for(String code:betcode.split("\\-")[1].replace("^", "").split(",")) {
			zhushu = zhushu + OTHER_2H_ZHUSHU[Integer.parseInt(code)];
		}
