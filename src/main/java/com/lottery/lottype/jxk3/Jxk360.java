...
//和值
//100960-04,05^
public class Jxk360 extends Jxk3X{
	
	private Jxk330 jxk330 = new Jxk330();
	...
	public String caculatePrizeLevel(String betcode, String wincode,
		...
		for (int code : codes) {
			if (code == num) {
				if (num == 4 || num == 17) {
					prize.append(LotteryDrawPrizeAwarder.JXK3_HZ_H2.value).append(",");
				} else if (num == 5 || num == 16) {
					prize.append(LotteryDrawPrizeAwarder.JXK3_HZ_H3.value).append(",");
				} else if (num == 6 || num == 15) {
					prize.append(LotteryDrawPrizeAwarder.JXK3_HZ_H4.value).append(",");
				} else if (num == 7 || num == 14) {
					prize.append(LotteryDrawPrizeAwarder.JXK3_HZ_H5.value).append(",");
				} else if (num == 8 || num == 13) {
					prize.append(LotteryDrawPrizeAwarder.JXK3_HZ_H7.value).append(",");
				} else if (num == 9 || num == 12) {
					prize.append(LotteryDrawPrizeAwarder.JXK3_HZ_H8.value).append(",");
				} else if (num == 10 || num == 11) {
					prize.append(LotteryDrawPrizeAwarder.JXK3_HZ_H9.value).append(",");
				}
			}
		}
		check2delete(prize);
	...
	public List<SplitedLot> splitByType(String betcode, int lotmulti,
		...
		for(SplitedLot s:list) {
			if(s.getBetcode().startsWith("101560")) {
				s.setAmt(getSingleBetAmount(s.getBetcode(), new BigDecimal(s.getLotMulti()), 200));
			}else if(s.getBetcode().startsWith("101530")){
				s.setAmt(jxk330.getSingleBetAmount(s.getBetcode(), new BigDecimal(s.getLotMulti()), 200));
			}
	...
	private List<SplitedLot> transform(String betcode,int lotmulti) {
		...
		for(String code:betcodes) {
			...
			if("3".equals(code)) {
				slot.setBetcode("101530-1,1,1^");
				slot.setAmt(200*lotmulti);
			}else if("18".equals(code)) {
				slot.setBetcode("101530-6,6,6^");
				slot.setAmt(200*lotmulti);
			} else {
				slot.setBetcode("101560-"+code+"^");
