...
public class Zucairenjiu01 extends ZucairenjiuX{
	...
	public long getSingleBetAmount(String betcode, BigDecimal beishu,
			int oneAmount) {
		if(!betcode.matches(regex_rjc01)) {
			throw new LotteryException(ErrorCode.betamount_error, ErrorCode.betamount_error.memo);
		}
		
		
		for(String betone:betcode.split("\\-")[1].split("\\^")) {
			int select = 0;
			String[] bets = betone.split(",");
	...
	private List<SplitedLot> transform(String betcode,int lotmulti) {
		...
		for(String innercode:betcodelist) {
			...
			if(sb.toString().split("\\^").length==3) {
				...
				slot.setBetcode("400201-"+sb.toString());
