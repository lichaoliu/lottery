...
public class Gdkl10SR2 extends Gdkl10X{
	...
	public String caculatePrizeLevel(String betcode, String wincode,int oneAmount) {
		...
		for(String code:betcode.split("\\^")) {
			if(totalHits(code, wincode)==2) {
				sb.append(LotteryDrawPrizeAwarder.GDK10_R2.value).append(",");
			}
		}
		if (sb.length() > 0) {
	...
	public List<SplitedLot> splitByType(String betcode, int lotmulti,
		...
		List<SplitedLot> zhumaList = transformSingle5(betcode,lotmulti,PlayType.gdkl10_sr2);
