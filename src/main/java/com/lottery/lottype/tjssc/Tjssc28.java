...
public class Tjssc28 extends TjsscX{

	@Override
	public String caculatePrizeLevel(String betcode, String wincode,
			int oneAmount) {
		String winb = Integer.parseInt(wincode.split(",")[2])>=5?"2":"1";
		String wins = wincode.split(",")[3];
		String wing = wincode.split(",")[4];
		
		betcode = betcode.replace("^", "").split("-")[1];
		
		String[] betsb = betcode.split(";")[0].split(",");
		String[] betss = betcode.split(";")[1].split(",");
		String[] betsg = betcode.split(";")[2].split(",");
			...
			for(String bets:betss) {
				for(String betg:betsg) {
					if (betb.toString().equals(winb)&& bets.toString().equals(wins) && betg.toString().equals(wing)) {
						prize.append(LotteryDrawPrizeAwarder.TJSSC_QW1.value);
						continue;
					}
					
					if (bets.toString().equals(wins) && betg.toString().equals(wing)) {
						prize.append(LotteryDrawPrizeAwarder.TJSSC_QW2.value);
						continue;
					}
				}
			}
		...
		for(String code0:betcodes[0].split(",")) {
			for(String code1:betcodes[1].split(",")) {
				for(String code2:betcodes[2].split(",")) {
					...
					lot.setBetcode("101628-"+code0+";"+code1+";"+code2+"^");
