...
public class Tjssc24 extends TjsscX{
	...
	public String caculatePrizeLevel(String betcode, String wincode,
		...
		if(isZusan(win)) {
			if(betcode.contains(wins[0])&&betcode.contains(wins[1])&&betcode.contains(wins[2])) {
				return LotteryDrawPrizeAwarder.TJSSC_Z3.value;
			}
		}
	...
	public long getSingleBetAmount(String betcode, BigDecimal beishu,
		...
		return MathUtils.combine(total, 2)*200*beishu.longValue()*2;
	}
	...
	public List<SplitedLot> splitByType(String betcode, int lotmulti,
		...
		for(SplitedLot s:list) {
			s.setBetcode("101624-"+sortBetcode(s.getBetcode().split("\\-")[1].replace("^", ""), ",")+"^");
