...
public class Tjssc07 extends TjsscX{
		...
		if(isZuliu(win)) {
			for(String code:betcode.split("-")[1].split("\\^")) {
				if(equalsIgnoreSort(code,win)) {
					prize.append(LotteryDrawPrizeAwarder.TJSSC_Z6.value).append(",");
				}
			}
	...
	private List<SplitedLot> transform(String betcode,int lotmulti) {
		...
		for(int i=0;i<betcodes.length;i++) {
			sb.append(sortBetcode(betcodes[i],",")).append("^");
			if(sb.toString().split("\\^").length==5) {
				SplitedLot slot = new SplitedLot(lotterytype);
				slot.setBetcode("101607-"+sb.toString());
