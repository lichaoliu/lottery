...
public class Hljkl10DR5 extends Hljkl10X{

	@Override
	public String caculatePrizeLevel(String betcode, String wincode,int oneAmount) {
		betcode = betcode.substring(7,betcode.length()-1);
		return caculatePrizeDR(betcode, wincode, 5, LotteryDrawPrizeAwarder.HLJK10_R5.value);
	}
	...
	public long getSingleBetAmount(String betcode, BigDecimal beishu,
			int oneAmount) {
		betcode = betcode.substring(7,betcode.length()-1);
		if(!validateBetcodeContainsLine(betcode,DR5)) {
			throw new LotteryException(ErrorCode.betamount_error, ErrorCode.betamount_error.memo);
		}
		if(betcode.replace("#", "").replace(",", "").length()<12) {
			throw new LotteryException(ErrorCode.betamount_error, ErrorCode.betamount_error.memo);
		}
		return MathUtils.combine(betcode.split("\\#")[1].split(",").length,
				5 - betcode.split("\\#")[0].split(",").length)
				* beishu.longValue()
				* 200;
	}
